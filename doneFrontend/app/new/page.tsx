// app/(workspace)/chat/new/page.tsx - Combined landing design with new chat functionality
'use client'
...
export default function NewChatPage() {
  ...
  useEffect(() => {
    if (initialMessage && !hasStartedChat) {
      setHasStartedChat(true)
      // The ChatWrapper will handle sending the message
    }
  }, [initialMessage, hasStartedChat])
  ...
  // Auto-send initial message from ChatWrapper
  const handleInitialSend = useCallback(async (text: string, context?: any) => {
    // This will be called by ChatWrapper when it mounts if we have an initial message
    // ChatWrapper will handle the actual sending and conversation creation
  }, [])
  ...
  // If we have an initial message or user has started chatting, show chat interface
  if (initialMessage || hasStartedChat) {
    return (
